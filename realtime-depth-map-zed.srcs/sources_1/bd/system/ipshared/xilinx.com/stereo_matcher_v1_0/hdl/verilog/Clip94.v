// ==============================================================
// RTL generated by Vivado(TM) HLS - High-Level Synthesis from C, C++ and SystemC
// Version: 2017.1
// Copyright (C) 1986-2017 Xilinx, Inc. All Rights Reserved.
// 
// ===========================================================

`timescale 1 ns / 1 ps 

module Clip94 (
        ap_clk,
        ap_rst,
        ap_start,
        ap_done,
        ap_continue,
        ap_idle,
        ap_ready,
        p_read,
        p_read1,
        dst_data_stream_V_dout,
        dst_data_stream_V_empty_n,
        dst_data_stream_V_read,
        cap_din,
        cap_full_n,
        cap_write
);

parameter    ap_ST_fsm_state1 = 3'd1;
parameter    ap_ST_fsm_pp0_stage0 = 3'd2;
parameter    ap_ST_fsm_state4 = 3'd4;

input   ap_clk;
input   ap_rst;
input   ap_start;
output   ap_done;
input   ap_continue;
output   ap_idle;
output   ap_ready;
input  [11:0] p_read;
input  [11:0] p_read1;
input  [15:0] dst_data_stream_V_dout;
input   dst_data_stream_V_empty_n;
output   dst_data_stream_V_read;
output  [7:0] cap_din;
input   cap_full_n;
output   cap_write;

reg ap_done;
reg ap_idle;
reg ap_ready;
reg dst_data_stream_V_read;
reg cap_write;

reg    ap_done_reg;
(* fsm_encoding = "none" *) reg   [2:0] ap_CS_fsm;
wire    ap_CS_fsm_state1;
reg    dst_data_stream_V_blk_n;
wire    ap_CS_fsm_pp0_stage0;
reg    ap_enable_reg_pp0_iter1;
wire    ap_block_pp0_stage0;
reg   [0:0] exitcond_flatten_reg_179;
reg    cap_blk_n;
reg   [22:0] indvar_flatten_reg_89;
wire   [22:0] bound_fu_168_p2;
reg   [22:0] bound_reg_174;
reg    ap_block_state1;
wire   [0:0] exitcond_flatten_fu_112_p2;
wire    ap_block_state2_pp0_stage0_iter0;
reg    ap_block_state3_pp0_stage0_iter1;
reg    ap_block_pp0_stage0_11001;
wire   [22:0] indvar_flatten_next_fu_117_p2;
reg    ap_enable_reg_pp0_iter0;
reg    ap_block_pp0_stage0_subdone;
reg    ap_condition_pp0_exit_iter0_state2;
reg    ap_block_pp0_stage0_01001;
wire   [10:0] tmp_fu_100_p1;
wire   [7:0] tmp_139_fu_135_p1;
wire   [0:0] tmp_88_i_fu_123_p2;
wire   [0:0] tmp_89_i_fu_129_p2;
wire   [0:0] tmp_s_fu_153_p2;
wire   [7:0] tmp_94_i_cast_fu_145_p3;
wire   [7:0] tmp_93_i_fu_139_p2;
wire   [11:0] bound_fu_168_p0;
wire   [10:0] bound_fu_168_p1;
wire    ap_CS_fsm_state4;
reg   [2:0] ap_NS_fsm;
reg    ap_idle_pp0;
wire    ap_enable_pp0;
wire   [22:0] bound_fu_168_p00;
wire   [22:0] bound_fu_168_p10;

// power-on initialization
initial begin
#0 ap_done_reg = 1'b0;
#0 ap_CS_fsm = 3'd1;
#0 ap_enable_reg_pp0_iter1 = 1'b0;
#0 ap_enable_reg_pp0_iter0 = 1'b0;
end

stereo_matcher_mufYi #(
    .ID( 1 ),
    .NUM_STAGE( 1 ),
    .din0_WIDTH( 12 ),
    .din1_WIDTH( 11 ),
    .dout_WIDTH( 23 ))
stereo_matcher_mufYi_U44(
    .din0(bound_fu_168_p0),
    .din1(bound_fu_168_p1),
    .dout(bound_fu_168_p2)
);

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_CS_fsm <= ap_ST_fsm_state1;
    end else begin
        ap_CS_fsm <= ap_NS_fsm;
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_done_reg <= 1'b0;
    end else begin
        if ((1'b1 == ap_continue)) begin
            ap_done_reg <= 1'b0;
        end else if ((1'b1 == ap_CS_fsm_state4)) begin
            ap_done_reg <= 1'b1;
        end
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_enable_reg_pp0_iter0 <= 1'b0;
    end else begin
        if (((1'b1 == ap_CS_fsm_pp0_stage0) & (ap_block_pp0_stage0_subdone == 1'b0) & (1'b1 == ap_condition_pp0_exit_iter0_state2))) begin
            ap_enable_reg_pp0_iter0 <= 1'b0;
        end else if (((1'b1 == ap_CS_fsm_state1) & ~((1'b0 == ap_start) | (ap_done_reg == 1'b1)))) begin
            ap_enable_reg_pp0_iter0 <= 1'b1;
        end
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_enable_reg_pp0_iter1 <= 1'b0;
    end else begin
        if (((ap_block_pp0_stage0_subdone == 1'b0) & (1'b1 == ap_condition_pp0_exit_iter0_state2))) begin
            ap_enable_reg_pp0_iter1 <= (ap_condition_pp0_exit_iter0_state2 ^ 1'b1);
        end else if ((ap_block_pp0_stage0_subdone == 1'b0)) begin
            ap_enable_reg_pp0_iter1 <= ap_enable_reg_pp0_iter0;
        end else if (((1'b1 == ap_CS_fsm_state1) & ~((1'b0 == ap_start) | (ap_done_reg == 1'b1)))) begin
            ap_enable_reg_pp0_iter1 <= 1'b0;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((1'b1 == ap_CS_fsm_pp0_stage0) & (ap_block_pp0_stage0_11001 == 1'b0) & (1'b1 == ap_enable_reg_pp0_iter0) & (1'd0 == exitcond_flatten_fu_112_p2))) begin
        indvar_flatten_reg_89 <= indvar_flatten_next_fu_117_p2;
    end else if (((1'b1 == ap_CS_fsm_state1) & ~((1'b0 == ap_start) | (ap_done_reg == 1'b1)))) begin
        indvar_flatten_reg_89 <= 23'd0;
    end
end

always @ (posedge ap_clk) begin
    if (((1'b1 == ap_CS_fsm_state1) & ~((1'b0 == ap_start) | (ap_done_reg == 1'b1)))) begin
        bound_reg_174 <= bound_fu_168_p2;
    end
end

always @ (posedge ap_clk) begin
    if (((1'b1 == ap_CS_fsm_pp0_stage0) & (ap_block_pp0_stage0_11001 == 1'b0))) begin
        exitcond_flatten_reg_179 <= exitcond_flatten_fu_112_p2;
    end
end

always @ (*) begin
    if ((exitcond_flatten_fu_112_p2 == 1'd1)) begin
        ap_condition_pp0_exit_iter0_state2 = 1'b1;
    end else begin
        ap_condition_pp0_exit_iter0_state2 = 1'b0;
    end
end

always @ (*) begin
    if ((1'b1 == ap_CS_fsm_state4)) begin
        ap_done = 1'b1;
    end else begin
        ap_done = ap_done_reg;
    end
end

always @ (*) begin
    if (((1'b0 == ap_start) & (1'b1 == ap_CS_fsm_state1))) begin
        ap_idle = 1'b1;
    end else begin
        ap_idle = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_enable_reg_pp0_iter0) & (1'b0 == ap_enable_reg_pp0_iter1))) begin
        ap_idle_pp0 = 1'b1;
    end else begin
        ap_idle_pp0 = 1'b0;
    end
end

always @ (*) begin
    if ((1'b1 == ap_CS_fsm_state4)) begin
        ap_ready = 1'b1;
    end else begin
        ap_ready = 1'b0;
    end
end

always @ (*) begin
    if (((1'b1 == ap_CS_fsm_pp0_stage0) & (1'b1 == ap_enable_reg_pp0_iter1) & (ap_block_pp0_stage0 == 1'b0) & (exitcond_flatten_reg_179 == 1'd0))) begin
        cap_blk_n = cap_full_n;
    end else begin
        cap_blk_n = 1'b1;
    end
end

always @ (*) begin
    if (((1'b1 == ap_CS_fsm_pp0_stage0) & (1'b1 == ap_enable_reg_pp0_iter1) & (exitcond_flatten_reg_179 == 1'd0) & (ap_block_pp0_stage0_11001 == 1'b0))) begin
        cap_write = 1'b1;
    end else begin
        cap_write = 1'b0;
    end
end

always @ (*) begin
    if (((1'b1 == ap_CS_fsm_pp0_stage0) & (1'b1 == ap_enable_reg_pp0_iter1) & (ap_block_pp0_stage0 == 1'b0) & (exitcond_flatten_reg_179 == 1'd0))) begin
        dst_data_stream_V_blk_n = dst_data_stream_V_empty_n;
    end else begin
        dst_data_stream_V_blk_n = 1'b1;
    end
end

always @ (*) begin
    if (((1'b1 == ap_CS_fsm_pp0_stage0) & (1'b1 == ap_enable_reg_pp0_iter1) & (exitcond_flatten_reg_179 == 1'd0) & (ap_block_pp0_stage0_11001 == 1'b0))) begin
        dst_data_stream_V_read = 1'b1;
    end else begin
        dst_data_stream_V_read = 1'b0;
    end
end

always @ (*) begin
    case (ap_CS_fsm)
        ap_ST_fsm_state1 : begin
            if (((1'b1 == ap_CS_fsm_state1) & ~((1'b0 == ap_start) | (ap_done_reg == 1'b1)))) begin
                ap_NS_fsm = ap_ST_fsm_pp0_stage0;
            end else begin
                ap_NS_fsm = ap_ST_fsm_state1;
            end
        end
        ap_ST_fsm_pp0_stage0 : begin
            if (~((1'b1 == ap_enable_reg_pp0_iter0) & (ap_block_pp0_stage0_subdone == 1'b0) & (exitcond_flatten_fu_112_p2 == 1'd1))) begin
                ap_NS_fsm = ap_ST_fsm_pp0_stage0;
            end else if (((1'b1 == ap_enable_reg_pp0_iter0) & (ap_block_pp0_stage0_subdone == 1'b0) & (exitcond_flatten_fu_112_p2 == 1'd1))) begin
                ap_NS_fsm = ap_ST_fsm_state4;
            end else begin
                ap_NS_fsm = ap_ST_fsm_pp0_stage0;
            end
        end
        ap_ST_fsm_state4 : begin
            ap_NS_fsm = ap_ST_fsm_state1;
        end
        default : begin
            ap_NS_fsm = 'bx;
        end
    endcase
end

assign ap_CS_fsm_pp0_stage0 = ap_CS_fsm[32'd1];

assign ap_CS_fsm_state1 = ap_CS_fsm[32'd0];

assign ap_CS_fsm_state4 = ap_CS_fsm[32'd2];

assign ap_block_pp0_stage0 = ~(1'b1 == 1'b1);

always @ (*) begin
    ap_block_pp0_stage0_01001 = ((1'b1 == ap_enable_reg_pp0_iter1) & (((exitcond_flatten_reg_179 == 1'd0) & (1'b0 == dst_data_stream_V_empty_n)) | ((exitcond_flatten_reg_179 == 1'd0) & (1'b0 == cap_full_n))));
end

always @ (*) begin
    ap_block_pp0_stage0_11001 = ((1'b1 == ap_enable_reg_pp0_iter1) & (((exitcond_flatten_reg_179 == 1'd0) & (1'b0 == dst_data_stream_V_empty_n)) | ((exitcond_flatten_reg_179 == 1'd0) & (1'b0 == cap_full_n))));
end

always @ (*) begin
    ap_block_pp0_stage0_subdone = ((1'b1 == ap_enable_reg_pp0_iter1) & (((exitcond_flatten_reg_179 == 1'd0) & (1'b0 == dst_data_stream_V_empty_n)) | ((exitcond_flatten_reg_179 == 1'd0) & (1'b0 == cap_full_n))));
end

always @ (*) begin
    ap_block_state1 = ((1'b0 == ap_start) | (ap_done_reg == 1'b1));
end

assign ap_block_state2_pp0_stage0_iter0 = ~(1'b1 == 1'b1);

always @ (*) begin
    ap_block_state3_pp0_stage0_iter1 = (((exitcond_flatten_reg_179 == 1'd0) & (1'b0 == dst_data_stream_V_empty_n)) | ((exitcond_flatten_reg_179 == 1'd0) & (1'b0 == cap_full_n)));
end

assign ap_enable_pp0 = (ap_idle_pp0 ^ 1'b1);

assign bound_fu_168_p0 = bound_fu_168_p00;

assign bound_fu_168_p00 = p_read1;

assign bound_fu_168_p1 = bound_fu_168_p10;

assign bound_fu_168_p10 = tmp_fu_100_p1;

assign cap_din = ((tmp_s_fu_153_p2[0:0] === 1'b1) ? tmp_94_i_cast_fu_145_p3 : tmp_93_i_fu_139_p2);

assign exitcond_flatten_fu_112_p2 = ((indvar_flatten_reg_89 == bound_reg_174) ? 1'b1 : 1'b0);

assign indvar_flatten_next_fu_117_p2 = (indvar_flatten_reg_89 + 23'd1);

assign tmp_139_fu_135_p1 = dst_data_stream_V_dout[7:0];

assign tmp_88_i_fu_123_p2 = (($signed(dst_data_stream_V_dout) < $signed(16'd65504)) ? 1'b1 : 1'b0);

assign tmp_89_i_fu_129_p2 = (($signed(dst_data_stream_V_dout) > $signed(16'd32)) ? 1'b1 : 1'b0);

assign tmp_93_i_fu_139_p2 = (8'd32 + tmp_139_fu_135_p1);

assign tmp_94_i_cast_fu_145_p3 = ((tmp_88_i_fu_123_p2[0:0] === 1'b1) ? 8'd0 : 8'd64);

assign tmp_fu_100_p1 = p_read[10:0];

assign tmp_s_fu_153_p2 = (tmp_88_i_fu_123_p2 | tmp_89_i_fu_129_p2);

endmodule //Clip94
